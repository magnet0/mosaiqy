...
&lt;script src="lib/mosaiqy-1.0.0.min.js" id="mosaiqy-tpl"&gt;&lt;/script&gt;</code></pre>
    ...
    <section id="how-to-use">
        ...
        <p>
            ...
            generated by a HTML5 markup (the rows highlighted) and the information about images (path of thumbnail/zoom and description)
...
&lt;/script&gt;</code></pre>
    ...
    <p>When you call the plugin you can specify some options (listed below). The most important one is <em>data</em>
    who accept an array of objects, each one containing the information about a single image (thumb and zoom).
    In this example, every object has  <em>img</em> and a  <em>desc</em> key (of course you can name your
    keys as you want). These key are dinamically replaced on the HTML5 template above
    respectively where the placeholders <em>${img}</em> and <em>${desc}</em> occur.</p>
    ...
    <ol>
        <li>The template <b>must</b> be defined inside a <code>&lt;script&gt;</code> element (I wrote mine in the element that loads the plugin, just to reduce code);
        <li>The script including template <b>must</b> define an <strong>id</strong> attribute specified as the <b>template</b> option value;
    ...
    <p>Of course you can also add other markup or hide the existing one via css but remember that
    the template code will be injected into a <code>&lt;li&gt;</code> element, so be sure to not wrap your template in
