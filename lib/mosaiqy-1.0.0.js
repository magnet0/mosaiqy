...
(function($) {
    ...
    Mosaiqy = function($) {
         ...
         * @description
         * 
         * <p>The main purpose is to properly increment the indexData for JSON
         * lookup. If user choosed "avoidDuplicate" option, then the method
         * checks if imag requested is already on stage. If so, a finite loop
         * look for the first image not in stage, increasing the dataIndex.</p>
         */
            ...
            init    : function(cnt, options) {
                ...
                if (imgToComplete) {
                    if (_s.data.length >= imgToComplete) {
                        ...
                        throw Error("Mosaiqy can't find missing images on JSON data.")
                    }
                }
